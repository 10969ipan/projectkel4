    ...
    <div>
        <nav class="flex mb-1" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1">
                ...
                <li><span class="text-xs text-gray-400">Tambah Pengguna Baru</span></li>
            </ol>
        </nav>
        <h1 class="text-xl font-bold text-gray-800 tracking-tight">Tambah Pengguna Baru</h1>
        <p class="text-xs text-gray-400 mt-0.5">Buat akun manajemen baru (Admin, Staff, atau Kustom)</p>
    </div>
    <a href="{{ route('users.index') }}" class="inline-flex items-center gap-1.5 text-xs font-semibold text-gray-500 hover:text-gray-700 bg-gray-100 hover:bg-gray-200 py-1.5 px-3 rounded-lg transition">
        ...
        <div class="p-6">
            <form action="{{ route('users.store') }}" method="POST" enctype="multipart/form-data" 
                  x-data="{ 
                      ...
                      permissions: {{ json_encode(old('menu_permissions', ['dashboard', 'items', 'transactions', 'item_requests', 'store_transactions', 'store_logs'])) }},
                      init() {
                          // If old role is a custom one, add it to list
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="password" class="block text-sm font-bold text-gray-700 mb-1">Kata Sandi (Password)</label>
                        ...
                        <div>
                            <label for="profile_photo" class="block text-sm font-bold text-gray-700 mb-1">Foto Profil</label>
                            <input type="file" name="profile_photo" id="profile_photo" accept="image/*"
                                class="mt-1 block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-55 focus:outline-none py-1.5 px-2">
